...
label define comuna 13201 "Puente Alto" 13101 "Santiago" 13119 "Maipu" 13110 "La Florida" 13401 "San Bernardo" 13122 "Peñalolen" 13125 "Quilicura" 13124 "Pudahuel" 
label values cod_com_alu comuna
...
gen n = 1
keep if migrant == 1
collapse (count) n_students = n, by(cod_com_alu pais_origen_alu)

// save for easy manipulation
*save "/Users/svaldiv/Dropbox/Research Ideas/1_Migration/Worked/Enrollment/enrollment_migrant_region.dta", replace

drop if cod_com_alu == 0 // those with no info
drop if pais_origen_alu == 0 // those with no info
...
*** (b) and reshape
reshape wide n_students, i(cod_com_alu) j(id)
...
order cod_com_alu n_students*

keep if cod_com_alu == 13201 | cod_com_alu == 13101 | cod_com_alu == 13119 | cod_com_alu == 13110 | cod_com_alu == 13401 | cod_com_alu == 13122 | cod_com_alu == 13125 | cod_com_alu == 13124
...
*graph bar (mean) n_students* , over(cod_com_alu, label(labsize(small)) axis(lcolor(none))) bargap(-20) ytitle("% Students", size(small)) ylabel(0(20) 100, labsize(small) format(%12.0fc)) blabel(bar, size(1.5) orientation(vertical) margin(vsmall) format(%12.0fc)) ysc(r(0(20)100)) legend(`legend' col(1) size(vsmall) pos(11) ring(0) region(fcolor(none))) xsize(2) ysize(1) asyvars percentages   
*graph export "$figures/Enrollment/migrant_comuna_RM_percent_2020.png", replace

   ** without chilean foreign students
   graph bar (mean) n_students1 n_students3 n_students4 n_students5 n_students6, ///
   over(cod_com_alu, label(labsize(small)) axis(lcolor(none))) bargap(-20)  ///
  ytitle("% Students", size(small)) ylabel(0(20) 100, labsize(small) format(%12.0fc)) ///
  blabel(bar, size(1.5) orientation(vertical) margin(vsmall) format(%12.0fc)) ysc(r(0(20)100)) ///
  legend(order(1 "Bolivia" 2 "Colombia" 3 "Haiti" 4 "Peru" 5 "Venezuela") col(1) size(vsmall) pos(11) ring(0) region(fcolor(none))) xsize(2) ysize(1) asyvars percentages 
  graph export "$figures/Enrollment/migrant_comuna_RM_percent_2020_notchile.png", replace
